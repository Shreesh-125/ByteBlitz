...
export const getAllContestWithPagination = async (page = 1, limit = 10) => {
  try {
    ...
    const response = await axios.get(
      `${import.meta.env.VITE_BACKEND_URI}/api/v1/contest?page=${page}&limit=${limit}`
    );
...
export const registerUser= async({contestId,userId})=>{
  const response=await axios.get(`${import.meta.env.VITE_BACKEND_URI}/api/v1/contest/register/${contestId}/${userId}`);
  return response;
}


export const getUserContests= async (username)=>{
  const response = await axios.get(`${import.meta.env.VITE_BACKEND_URI}/api/v1/user/${username}/contests`);
